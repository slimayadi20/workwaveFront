...
export class CalendarService {
  ...
  addEvent(body: any) {
    return this.http.post("http://localhost:8082" + '/event', body, {
      observe: 'body',
 ...
 /* updateEvent(event: any): Observable<any> {
  ...
  updateEvent(body: any) {
    return this.http.put("http://localhost:8082" + '/event', body, {
      observe: 'body',
      headers: new HttpHeaders().append('Content-Type', 'application/json')
    });
  }
  ...
  removeEvent(id: number) {
    return this.http.delete(`http://localhost:8082/event/${id}`, {
      observe: 'body',
      headers: new HttpHeaders().append('Content-Type', 'application/json')
    });
  }
